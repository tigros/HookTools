	...
	)
{
	...
	a_hi = 0, b_hi = 0;
	while ((a_hi < a->hook_count) && (b_hi < b->hook_count) && (dcount == 0))
	{
		int ret = compare_hook(&a->hook[a_hi], &b->hook[b_hi]);
		...
		else
		{
			...
			*/
			if (!a->hook[a_hi].ignore || !b->hook[b_hi].ignore)
				if (print_diff_hook(&a->hook[a_hi], &b->hook[b_hi], deskname))
					dcount++;
	...
	)
{
	...
	for (a = list_a->head, b = list_b->head; (a && b && !dcount); a = a->next, b = b->next)
		dcount += diffcount(a, b);
